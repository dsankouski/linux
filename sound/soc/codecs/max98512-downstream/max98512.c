				    ...
				    struct snd_ctl_elem_value *ucontrol)
{
	...
	struct max98512_priv *max98512 = snd_soc_component_get_drvdata(codec);
#else
				...
				unsigned int fmt)
{
	...
	struct max98512_priv *max98512 = snd_soc_component_get_drvdata(codec_dai->component);
#else
	struct snd_soc_component *codec = codec_dai->codec;
	struct max98512_priv *max98512 = snd_soc_codec_get_drvdata(codec);
#endif
	unsigned int mode = 0;
			      ...
			      struct snd_pcm_hw_params *params)
{
	...
	struct snd_soc_component *codec = max98512->codec;
...
static const char * const max98512_spk_analog_mute_text[] = {
	"On", "Off"
};
...
static int max98512_analog_mute_get(struct snd_kcontrol *kcontrol,
				 struct snd_ctl_elem_value *ucontrol)
{
	return 0;
}

static int max98512_analog_mute_put(struct snd_kcontrol *kcontrol,
				 struct snd_ctl_elem_value *ucontrol)
{
#if LINUX_VERSION_CODE >= KERNEL_VERSION(4,18,0)
	struct snd_soc_component *codec = snd_soc_kcontrol_component(kcontrol);
	...
	msg_maxim("set spk_gain_mute [%d] ", enable);
...
static const struct soc_enum max98512_enum[] = {
	...
	SOC_ENUM_SINGLE_EXT(ARRAY_SIZE(max98512_spk_analog_mute_text),
				max98512_spk_analog_mute_text),
};
...
static const struct snd_kcontrol_new max98512_snd_controls[] = {
	...
	SOC_ENUM_EXT("SPK Analog Mute", max98512_enum[6],
		     max98512_analog_mute_get, max98512_analog_mute_put),
...
#if LINUX_VERSION_CODE >= KERNEL_VERSION(4,18,0)
static int max98512_probe(struct snd_soc_component *codec)
{
	struct max98512_priv *max98512 = snd_soc_component_get_drvdata(codec);
#else
static int tfa98xx_probe(struct snd_soc_codec *codec)
{
...
#if LINUX_VERSION_CODE >= KERNEL_VERSION(4,18,0)
	snd_soc_component_init_regmap(codec, max98512->regmap_l);
#endif
	struct max98512_pdata *pdata = max98512->pdata;
	struct max98512_volume_step_info *vstep = &max98512->vstep;
	int ret = 0, reg = 0x0;
	...
	ret = snd_soc_add_component_controls(codec,
		max98512_snd_controls, ARRAY_SIZE(max98512_snd_controls));
#else
...
#if defined(USE_DSM_LOG) || defined(USE_DSM_UPDATE_CAL)
	if (!g_class)
		g_class = class_create(class_name_log);
...
static const struct snd_soc_component_driver soc_codec_dev_max98512 = {
...
struct i2c_client *max98512_add_sub_device(int bus_id, int slave_addr)
{
	...
	if (adapter) {
		i2c = i2c_new_client_device(adapter, max98512_i2c_sub_board);
		if (i2c)
...
static int max98512_i2c_probe(struct i2c_client *i2c)
{
	...
	ret = devm_snd_soc_register_component(&i2c->dev,
				&soc_codec_dev_max98512, max98512_dai,
...
err_alloc_priv:
	msg_maxim("Failed with %d.", ret);

	return ret;
}

static void max98512_i2c_remove(struct i2c_client *client)
{
