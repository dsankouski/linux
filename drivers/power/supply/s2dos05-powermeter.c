...
static enum power_supply_property s2dos05_props[] = {
...
static void s2m_adc_read_data(struct device *dev, int channel)
{
	...
	if (adc_meter->adc_sync_mode == ASYNC_MODE) {
		...
		s2dos05_update_reg(adc_meter->i2c, S2DOS05_REG_PWRMT_CTRL1, ADC_ASYNCRD_MASK, ADC_ASYNCRD_MASK);
		...
		if (channel < 0) {
			for (i = 0; i < 8; i++) {
				s2dos05_update_reg(adc_meter->i2c, S2DOS05_REG_PWRMT_CTRL2,
							i, ADC_PTR_MASK);
				s2dos05_read_reg(adc_meter->i2c, S2DOS05_REG_PWRMT_DATA,
							&data_l);
...
static ssize_t adc_val_0_show(struct device *dev, struct device_attribute *attr, char *buf)
{
	...
	if (adc_meter->adc_mode == POWER_METER)
		return snprintf(buf, PAGE_SIZE, "%d uW\n", (adc_meter->adc_val[0] * get_coeff(dev, 0))/100);
	else
		return snprintf(buf, PAGE_SIZE, "%d uA\n", adc_meter->adc_val[0] * get_coeff(dev, 0));
}
...
static ssize_t adc_val_1_show(struct device *dev, struct device_attribute *attr, char *buf)
{
	s2m_adc_read_data(dev, 1);
	if (adc_meter->adc_mode == POWER_METER)
		return snprintf(buf, PAGE_SIZE, "%d uW\n", (adc_meter->adc_val[1] * get_coeff(dev, 1))/100);
	else
		return snprintf(buf, PAGE_SIZE, "%d uA\n", adc_meter->adc_val[1] * get_coeff(dev, 1));
}
...
static ssize_t adc_val_2_show(struct device *dev, struct device_attribute *attr, char *buf)
{
	...
	if (adc_meter->adc_mode == POWER_METER)
		return snprintf(buf, PAGE_SIZE, "%d uW\n", (adc_meter->adc_val[2] * get_coeff(dev, 2))/100);
	else
		return snprintf(buf, PAGE_SIZE, "%d uA\n", adc_meter->adc_val[2] * get_coeff(dev, 2));
}
...
static ssize_t adc_val_3_show(struct device *dev, struct device_attribute *attr, char *buf)
{
	...
	if (adc_meter->adc_mode == POWER_METER)
		return snprintf(buf, PAGE_SIZE, "%d uW\n", (adc_meter->adc_val[3] * get_coeff(dev, 3))/100);
	else
		return snprintf(buf, PAGE_SIZE, "%d uA\n", adc_meter->adc_val[3] * get_coeff(dev, 3));
}
...
static ssize_t adc_val_4_show(struct device *dev, struct device_attribute *attr, char *buf)
{
	...
	if (adc_meter->adc_mode == POWER_METER)
		return snprintf(buf, PAGE_SIZE, "%d uW\n", (adc_meter->adc_val[4] * get_coeff(dev, 4))/100);
	else
		return snprintf(buf, PAGE_SIZE, "%d uA\n", adc_meter->adc_val[4] * get_coeff(dev, 4));
}
...
static ssize_t adc_val_5_show(struct device *dev, struct device_attribute *attr, char *buf)
{
	...
	if (adc_meter->adc_mode == POWER_METER)
		return snprintf(buf, PAGE_SIZE, "%d uW\n", (adc_meter->adc_val[5] * get_coeff(dev, 5))/100);
	else
		return snprintf(buf, PAGE_SIZE, "%d uA\n", adc_meter->adc_val[5] * get_coeff(dev, 5));
}
...
static ssize_t adc_val_6_show(struct device *dev, struct device_attribute *attr, char *buf)
{
	...
	if (adc_meter->adc_mode == POWER_METER)
		return snprintf(buf, PAGE_SIZE, "%d uW\n", (adc_meter->adc_val[6] * get_coeff(dev, 6))/100);
	else
		return snprintf(buf, PAGE_SIZE, "%d uA\n", adc_meter->adc_val[6] * get_coeff(dev, 6));
}
...
static ssize_t adc_val_7_show(struct device *dev, struct device_attribute *attr, char *buf)
{
	...
	if (adc_meter->adc_mode == POWER_METER)
		return snprintf(buf, PAGE_SIZE, "%d uW\n", (adc_meter->adc_val[7] * get_coeff(dev, 7))/100);
	else
		return snprintf(buf, PAGE_SIZE, "%d uA\n", adc_meter->adc_val[7] * get_coeff(dev, 7));
}

static void adc_ctrl1_update(struct device *dev)
{
	/* ADC temporarily off */
	s2dos05_update_reg(adc_meter->i2c, S2DOS05_REG_PWRMT_CTRL2, 0x00, ADC_EN_MASK);

	/* update ADC_CTRL1 register */
	s2dos05_write_reg(adc_meter->i2c, S2DOS05_REG_PWRMT_CTRL1, adc_meter->adc_ctrl1);

	/* ADC Continuous ON */
	s2dos05_update_reg(adc_meter->i2c, S2DOS05_REG_PWRMT_CTRL2, ADC_EN_MASK, ADC_EN_MASK);
}
...
static int s2dos05_get_prop(struct power_supply *psy,
			       enum power_supply_property psp,
			       union power_supply_propval *val)
{
	struct s2dos05_dc_info *info = power_supply_get_drvdata(psy);
...
void s2dos05_powermeter_init(struct s2dos05_dev *s2dos05)
{
	...
	adc_meter->adc_val = kzalloc(sizeof(u16)*S2DOS05_MAX_ADC_CHANNEL, GFP_KERNEL);
