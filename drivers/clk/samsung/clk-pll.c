...
/* Maximum lock time can be ???? * PDIV cycles */
				...
				const struct pwrcal_pll_rate_table  *rate_table)
{
	...
	pwrcal_setbit(CMU_MIF0_CTRL, 0, 0x0); // MIF clock control from local CMU
	pwrcal_setbit(CMU_MIF1_CTRL, 0, 0x0); // MIF clock control from local CMU
	pwrcal_writel(clk->offset, pll_con0);
